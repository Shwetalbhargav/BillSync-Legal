...
window.addEventListener('hashchange', openBillSyncComposeIntent);
...
async function requestGeneratedEmail(prompt) {
  const data = await backendRequest('/api/ai/generate-email', {
    method: 'POST',
    body: { prompt },
    timeoutMs: 45000,
  });
...
function showAIPromptPopup(composeRoot) {
  ...
  popup.innerHTML = `
    ...
    <div id="lb-ai-body" style="padding: 10px 12px; overflow-y: auto;">
      ...
      <div style="margin-top:10px; display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
        <button id="lb-ai-generate" style="background:#1677ff;color:#fff;border:none;padding:8px 12px;border-radius:6px;cursor:pointer;">Generate</button>
        <button id="lb-ai-insert-send" style="background:#0a7d32;color:#fff;border:none;padding:8px 12px;border-radius:6px;cursor:pointer;">Insert & Send</button>
  ...
  Object.assign(popup.querySelector('#lb-ai-generate').style, {
    background: '#1a73e8',
    padding: '7px 12px',
    borderRadius: '4px',
    font: '500 13px Arial, sans-serif',
  });
  Object.assign(popup.querySelector('#lb-ai-insert-send').style, {
    background: '#137333',
    padding: '7px 12px',
    borderRadius: '4px',
    font: '500 13px Arial, sans-serif',
  ...
  const btnGenerate = popup.querySelector('#lb-ai-generate');
  const btnInsertSend = popup.querySelector('#lb-ai-insert-send');
  promptEl.value = LB_PROMPT_FROM_URL || window.lb_ai_prompt || '';

  async function generateAndInsert({ sendAfter = false } = {}) {
    ...
    try {
      ...
      statusEl.textContent = sendAfter ? 'Inserted - sending...' : 'Inserted.';
      if (sendAfter) {
        const sendBtn =
          getGmailAdapter()?.getSendButton?.(composeRoot) ||
          composeRoot.querySelector('[data-tooltip*="Send"], [aria-label*="Send"]') ||
          null;
        if (!clickGmailSendButton(sendBtn)) {
          statusEl.textContent = 'Inserted, but Send button not found.';
        }
      }
    } catch (e) {
      ...
      btnInsertSend.disabled = false;
    }
  }

  btnGenerate.onclick = () => generateAndInsert({ sendAfter: false });
  btnInsertSend.onclick = () => generateAndInsert({ sendAfter: true });
}
