...
const ACTIVE_WORK_SESSION_KEY = 'lbActiveWorkSession';
const WORK_SESSION_CACHE_MS = 10000;
let activeWorkSessionCache = { at: 0, data: null };
...
function buildBackendUrl(baseUrl, path) {
  const origin = normalizeBackendBaseUrl(baseUrl);
...
async function refreshExtensionAuthSession() {
  ...
  const response = await fetchWithTimeout(buildBackendUrl(syncValues.backendBaseUrl, '/api/auth/extension-token'), {
    method: 'POST',
...
async function sendBackendRequest(request = {}) {
  ...
  const requestUrl = buildBackendUrl(backendBaseUrl, path);
...
async function sanitizeStoredBackendSettings() {
  const values = await storageSyncGet(['backendBaseUrl']);
  if (!values.backendBaseUrl) return;
  const normalized = normalizeBackendBaseUrl(values.backendBaseUrl);
  if (normalized !== values.backendBaseUrl) {
    await storageSyncSet({ backendBaseUrl: normalized });
    await logEvent('backend_base_url_normalized', { backendBaseUrl: normalized });
  }
}
...
async function getCurrentWorkSession({ force = false } = {}) {
  const cached = activeWorkSessionCache.data;
  if (!force && cached && Date.now() - activeWorkSessionCache.at < WORK_SESSION_CACHE_MS) {
    return cached;
  }
  const result = await sendBackendRequest({
    path: '/api/work-sessions/current',
    method: 'GET',
    queueOnFailure: false,
    timeoutMs: 10000,
  });
  if (!result.ok) return null;
  const session = result.data?.data || result.data || null;
  activeWorkSessionCache = { at: Date.now(), data: session || null };
  await storageLocalSet({ [ACTIVE_WORK_SESSION_KEY]: session || null });
  return session || null;
}

async function updateWorkSession(id, action, body = {}) {
  const workSessionId = String(id || '').trim();
  if (!workSessionId) return { ok: false, status: 400, error: 'workSessionId is required.' };
  const result = await sendBackendRequest({
    path: `/api/work-sessions/${encodeURIComponent(workSessionId)}/${action}`,
    method: 'POST',
    body,
    queueOnFailure: false,
    timeoutMs: 15000,
  });
  activeWorkSessionCache = { at: 0, data: null };
  await getCurrentWorkSession({ force: true }).catch(() => null);
  return result;
}
...
chrome.runtime.onStartup.addListener(() => {
  scheduleRetry();
  sanitizeStoredBackendSettings().catch((err) => logEvent('backend_url_sanitize_failed', { error: err.message }));
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message?.type === 'LB_CURRENT_WORK_SESSION') {
    getCurrentWorkSession({ force: Boolean(message.force) })
      .then((session) => sendResponse({ ok: true, data: session }))
      .catch((err) => sendResponse({ ok: false, error: err.message || 'Current work session unavailable' }));
    return true;
  }

  if (message?.type === 'LB_WORK_SESSION_COMMAND') {
    const action = String(message.action || '').toLowerCase();
    const allowed = new Set(['pause', 'resume', 'stop', 'discard']);
    if (!allowed.has(action)) {
      sendResponse({ ok: false, status: 400, error: 'Unsupported work session command.' });
      return true;
    }
    updateWorkSession(message.workSessionId, action, message.body || {})
      .then((result) => sendResponse(result))
      .catch((err) => sendResponse({ ok: false, status: 0, error: err.message || 'Work session command failed' }));
    return true;
  }
