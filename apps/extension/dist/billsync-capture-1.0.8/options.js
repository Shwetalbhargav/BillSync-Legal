...
function setStatus(message, isError = false) {
  statusEl.textContent = formatMessage(message);
...
function formatMessage(value, fallback = '') {
  ...
  return value.message || value.error || value.raw || JSON.stringify(value);
}
...
function loadSettings() {
  storageGet(EXTENSION_STORAGE_KEYS, (values) => {
    ...
    backendBaseUrlInput.value = normalizeBackendInput(values.backendBaseUrl || '');
...
function saveSettings() {
  ...
  const backendBaseUrl = normalizeBackendInput(backendBaseUrlInput.value || DEFAULT_BACKEND);
...
async function openBackendLogin() {
  ...
  if (!response.ok) {
    authStatusEl.textContent = formatMessage(response.error || response.data, 'Could not open backend login.');
...
async function checkBackendSession() {
  ...
  if (!response.ok) {
    authStatusEl.textContent = response.status === 401
      ...
      : formatMessage(response.error || response.data, 'Session check failed.');
    authStatusEl.style.color = '#b42318';
    return;
  }
