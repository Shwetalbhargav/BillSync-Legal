...
function normalizeArray(values = []) {
  return Array.isArray(values) ? values.filter(Boolean).map((value) => String(value).trim().toLowerCase()) : [];
}
...
function resolveModuleState({ module, access, workspaceModule, permissionAllowed, dependencyIssues }) {
...
export function buildNavigationModel({ modules = [], permissions = [], accessByModule = {}, workspaceModules = {} }) {
  ...
  for (const module of [...modules].sort((a, b) => (a.order || 100) - (b.order || 100))) {
    const dependencyIssues = (module.dependencies || [])
